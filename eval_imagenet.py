...
import pandas as pd
import time
ds = robustness.datasets.DATASETS['imagenet']('/datasets/ImageNet2012')
...
for model_name in ['resnetv2_50', 'vit_base_patch16_224', 'mobilenetv2_100', 'efficientnet_b0']:
    ...
    res_ada = np.r_[res_ada, np.array([[num_hold/n_test, num_violation/n_test, num_None/n_test, total_num/n_test]])]
    ...
    np.savez('./'+model_name+'_ada',time_ac=np.array(time_ada), result_ac=np.array(result_ada)) 