...
STEPS = {
    ...
    'color': attack_steps.Color,
    'blur': attack_steps.Blur,
    'semantic': attack_steps.Semantic,
...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, tries, rot=30, trans=0.3, scale=0.3, 
                hue=0, satu=0, bright=0, cont=0, 
                gau_size=11, gau_sigma=0, 
                attack_type='random', transform_type='spatial', with_latent=False,
        ...
        if transform_type=='spatial':
            step = STEPS['spatial'](attack_type=attack_type,
                                rot=rot, trans=trans, scale=scale)
        
        if transform_type=='color':
            step = STEPS['color'](attack_type=attack_type,
                                hue=hue, satu=satu, bright=bright, cont=cont)

        if transform_type=='blur':
            step = STEPS['blur'](attack_type=attack_type,
                                gau_size=gau_size, gau_sigma=gau_sigma)
        if transform_type=='semantic':
            step = STEPS['semantic'](attack_type=attack_type, rot=rot, trans=trans, scale=scale, 
                                    hue=hue, satu=satu, bright=bright, cont=cont,
                                    gau_size=gau_size, gau_sigma=gau_sigma)
