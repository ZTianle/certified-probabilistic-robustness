...
def make_loaders(workers, batch_size, transforms, data_path, data_aug=True, 
    ...
    **INTERNAL FUNCTION**
    ...
    if test_subset is not None:
        rng = np.random.RandomState(seed)
        indices = ch.randperm(len(test_set))[:test_subset_size]
        test_set = Subset(test_set, indices)
        
    test_loader = DataLoader(test_set, batch_size=val_batch_size, shuffle=True, num_workers=workers, worker_init_fn=worker_init_fn, pin_memory=True)
